...
default_joint_action = {
    MMK2Components.LEFT_ARM: JointState(position=[0.0, -0.166, 0.032, 0., 1.571, 2.223]),
    MMK2Components.RIGHT_ARM: JointState(position=[-0.0, -0.166 , 0.032, 0.0, -1.571, -2.223]),
        ...
        for comp, images in comp_images.items():
            for img_type, img in images.data.items():
                if img.shape[0] == 1:
                    ...
                    break
            try:
                cam_publishers_node.update_image(cam_mapping[comp.value], img)
